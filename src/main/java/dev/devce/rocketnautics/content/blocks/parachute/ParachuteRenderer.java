...
public class ParachuteRenderer extends SafeBlockEntityRenderer<ParachuteCaseBlockEntity> {
    ...
    protected void renderSafe(ParachuteCaseBlockEntity be, float partialTicks,
        ...
        if (be.lastRenderTime == 0)
            be.lastRenderTime = time;
        float dt = (time - be.lastRenderTime) / 50.0f; // roughly 20 ticks per second
        be.lastRenderTime = time;
        if (dt > 2.0f)
            dt = 2.0f; // cap to prevent jumps

        boolean falling = be.isFalling();
        if (dt > 0) {
            float targetY = falling ? 8.0f : -15.0f; // Fall through ship when collapsed
            float accel = (targetY - be.canopyY) * (falling ? 0.2f : 0.8f);
            if (!falling)
                accel -= 0.4f; // Smoother gravity pull when collapsing

            be.canopyVelocity += accel * dt;
            be.canopyVelocity *= (float) Math.pow(falling ? 0.7 : 0.6, dt);
            be.canopyY += be.canopyVelocity * dt;

            // Clamp canopyY based on ground height (minLocalY)
            float visualMinY = be.minLocalY;
            if (be.canopyY < visualMinY) {
                be.canopyY = visualMinY;
                be.canopyVelocity = 0.0f;
            }
        ...
        // In modern Minecraft, the PoseStack inside a BlockEntityRenderer is already
        // aligned
        // to the world axes (it does not contain the camera rotation, only the Ship's
        // rotation).
        ...
        // 3. Canopy center in local space (starts at 0.5, 1.0, 0.5, then goes UP in the
        // world)
    ...
    private static final ResourceLocation ROPE_TEX = ResourceLocation.fromNamespaceAndPath("minecraft",
    ...
    private static void renderRopeSegment(PoseStack ms, MultiBufferSource buffer,
        ...
        float w = 0.015f; // thin rope width
        float h = len * 0.5f;
    ...
    private static void addVertex(VertexConsumer vc, PoseStack.Pose pose, float x, float y, float z, float u, float v,
            int light, int overlay) {
    ...
    private static void renderBlock(PoseStack ms, MultiBufferSource buffer,
        ...
        ms.pushPose();
        // Since canopyCenterLocal already placed us at the precise center of a block
        // (0.5),
        // we subtract 0.5f so the block's true center visually lands on our integer
        // grid points
